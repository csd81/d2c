# Phase 5: Compaction Pipeline — Implementation Plan

## Context

Phases 1-4 complete (168 tests). LoopConfig has `compact_config: Any = None` placeholder. The loop already has a `"Phase 5 will add compaction pipeline here"` comment at the context shaping stage. The config module already has `tool_result_max_chars`, `pressure_threshold`, and `context_window_tokens` fields.

## Files to Create/Modify

### 1. CREATE `src/d2c/compact.py`

**CompactConfig** dataclass:
- `tool_result_max_chars: int = 30_000` — per-tool-result budget cap
- `pressure_threshold: float = 0.85` — fraction of context window that triggers compaction
- `context_window_tokens: int = 128_000` — model context window
- `chars_per_token: float = 3.5` — rough estimate for token counting

**Functions:**
- `applyContextShapers(messages, compact_config)` — orchestrator: runs budget reduction, then auto-compact if still over pressure limit. Entry point called by queryLoop before model call.
- `applyBudgetReduction(messages, config)` — caps individual tool result strings to `tool_result_max_chars`. Adds truncation notice with original size.
- `estimate_tokens(messages, config)` — total character count of all messages divided by chars_per_token.
- `compute_pressure_limit(config)` — `context_window_tokens * pressure_threshold`.
- `getCompactPrompt(messages)` — format all but last 2 turns as a single prompt string for the compaction model.
- `buildPostCompactMessages(original_messages, summary)` — keeps system messages, replaces old history with summary as user message, keeps last 4 messages for continuity.
- `autoCompact(messages, loop_config)` — async: builds compact prompt, calls model (no tools), returns post-compact messages. Records compact_boundary via session_store.

### 2. MODIFY `src/d2c/loop.py`

- Import `applyContextShapers`, `CompactConfig` from compact
- Replace the `# Phase 5 will add compaction pipeline here` comment with actual call: `messages_for_query = applyContextShapers(state.messages, loop_config.compact_config)`
- Pass `compact_config` into LoopConfig when building in main.py

### 3. MODIFY `src/d2c/main.py`

- Create `CompactConfig` from config values and pass to `LoopConfig(compact_config=...)`

### 4. CREATE `tests/test_compact.py`

Tests (~15):
- `applyBudgetReduction` caps long tool outputs, preserves short ones
- `applyBudgetReduction` handles non-tool messages (pass through)
- `estimate_tokens` returns reasonable estimate
- `compute_pressure_limit` calculates correctly
- `getCompactPrompt` formats messages, excludes last 4
- `buildPostCompactMessages` preserves system + recent, includes summary
- `buildPostCompactMessages` with < 4 messages: does not exclude any
- `applyContextShapers` only applies budget reduction when under threshold
- `applyContextShapers` applies both shapers when over threshold (mocked autoCompact)
- Edge: empty messages, no tool messages, mixed content types

## Verification

1. `python -m pytest tests/test_compact.py -v` — all compaction tests pass
2. `python -m pytest tests/ -v` — full suite passes (168 existing + new)

## Context

Phases 1-3 are complete: tools, agent loop, context assembly, and permission system all pass (132 tests). LoopConfig already has `session_store: Any = None` placeholder. Now need to implement append-only JSONL transcripts, session resume, and session fork per paper Section 9.

## Files to Create/Modify

### 1. CREATE `src/d2c/persistence.py`

**SessionEntry** — one JSONL line representing a single event:
- `role`: "user" | "assistant" | "tool" | "system"
- `content`: str | list[dict] (matches internal message format)
- `timestamp`: ISO 8601 UTC
- `entry_type`: "message" | "compact_boundary" | "subagent_summary"
- `metadata`: dict (tool_name, tool_use_id, event type, etc.)
- `to_jsonl_line()`: serialize to JSON string

**SessionStore** — append-only JSONL transcript:
- `__init__(base_dir, session_id, project_dir)`: sets up paths
- `append(entry)`: write one line, create dirs if needed
- `read_transcript()`: parse all JSONL lines back to SessionEntry list
- `reconstruct_messages(entries)`: rebuild API-compatible message array; handles compact_boundary markers (resets messages array when boundary found)
- `get_sidechain_path(subagent_id)`: returns path for subagent sidechain (Phase 8)
- `append_compact_boundary(preserved_head_uuid)`: writes boundary marker (Phase 5)

**SessionManager** — session lifecycle:
- `create_session(cwd)`: generate 8-char session_id, create SessionStore, write session_start entry
- `resume_session(session_id, cwd)`: read transcript, reconstruct messages. IMPORTANT: permissions NOT restored per paper Section 9.2
- `fork_session(source_id, cwd)`: copy transcript entries to new session, write forked_from marker

**Key design details:**
- Session dir: `~/.d2c/sessions/{session_id}.jsonl`
- JSONL format: one JSON object per line, no trailing commas
- Thread safety: simple file append (no multi-process locking needed for CLI)
- Malformed lines: skip on read, log warning

### 2. MODIFY `src/d2c/loop.py`

Add session recording to `queryLoop()`:
- After user message appended to state.messages → record as SessionEntry (role="user")
- After assistant response (text or tool_use) → record as SessionEntry (role="assistant")
- After each tool result appended → record as SessionEntry (role="tool", with tool metadata)
- After loop stops → record stop event as SessionEntry (role="system", entry_type="message", metadata={"stop_reason": ...})

All recording is conditional: `if loop_config.session_store: loop_config.session_store.append(...)`. The session_store remains optional (None by default).
...
In `run_headless()` and `run_interactive()`:
- Create `SessionManager` instance at startup
- If `--resume`: call `resume_session()`, use reconstructed messages as initial history
- If `--fork`: call `fork_session()` to create new session from existing one
- Otherwise: call `create_session()` for new session
- Wire `session_store` into `LoopConfig`

Session info displayed at startup: "Session: abc12345"

### 4. CREATE `tests/test_persistence.py`

Tests:
- SessionEntry serialization/deserialization via JSONL round-trip
- SessionStore append → file contains JSONL line
- SessionStore read_transcript → returns correct entries
- reconstruct_messages: converts entries to API message format
- reconstruct_messages: compact_boundary resets message array
- SessionManager.create_session: returns valid store, session_start entry written
- SessionManager.resume_session: returns messages array
- SessionManager.fork_session: copies entries, new session_id
- Sidechain path is subdirectory of session dir
- Malformed JSONL line skipped on read
- SessionStore handles non-existent transcript (returns empty list)

## Verification

1. `python -m pytest tests/test_persistence.py -v` — all persistence tests pass
2. `python -m pytest tests/ -v` — full suite passes (132 existing + new)
3. Manual: `python -m d2c --session test1 "hello"` creates `.d2c/sessions/test1.jsonl`
4. Manual: `python -m d2c --resume test1 "continue"` resumes from transcript
...
- `anthropic` — model API
- `pydantic` — tool input validation (model-based parsing)
- `httpx` — WebFetch/WebSearch, MCP HTTP transport
...
10. **Full suite**: `pytest tests/` — all 8 test files passing
