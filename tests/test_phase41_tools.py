...
async def test_replace_many_atomic_and_checkpoints(tmp_dir):
    ...
    bad = await ReplaceManyTool().execute(
        file_path=str(f),
        replacements=[
    ...
    ok = await ReplaceManyTool().execute(
        file_path=str(f),
        replacements=[
            {"old_string": "a = 1", "new_string": "a = 9"},
...
async def test_json_edit_set_delete_and_invalid(tmp_dir):
    ...
    r1 = await JsonEditTool().execute(
        file_path=str(f), operation="set", path="scripts.test", value="pytest"
    )
    assert not r1.error
